...
points <- read.csv(file = "/home/xiongh/Biomod2/GYLW_all.csv", header = TRUE)
...
# List all environmental raster files in ASC format.
envtList <- list.files(
  path = "/home/xiongh/Biomod2/new/result_1/env/",
  pattern = ".asc"
)

print(envtList)

# Set the working directory to the environmental raster folder.
...
write.csv(eval_df, "./re1__all.csv")
...
my_projection <- BIOMOD_Projection(
  ...
  new.env = envt.st,
  proj.name = "Maxent_Projection_re1all",
